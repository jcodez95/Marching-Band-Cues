...
## Status: Bug fix — cue sheet import not reconnecting on iOS

**If you already deployed this app and someone imported a cue sheet
before this fix**, that entry is stuck with an old, incompatible
fingerprint format and won't reconnect even after updating — **delete it
from the library (new 🗑 button on each entry) and import the cue sheet
again.** Freshly imported/loaded files use the corrected format and don't
need this.

**What was wrong:** the file-matching fingerprint used to include the
audio's duration. Duration is measured by decoding the file, and
different browsers/platforms can report meaningfully different durations
for the exact same bytes (especially MP3, where duration is often
estimated rather than stored reliably) — enough that iOS loading a file
that was exported/tested elsewhere would compute a different fingerprint
than expected, leaving a correctly-imported cue sheet permanently stuck on
"Needs re-select." Fixed by dropping duration from the fingerprint
entirely (`fingerprint.js`) — filename + exact byte size alone is already
essentially collision-proof for this use case, and doesn't drift across
platforms. See "How cue sheet sharing works" below for the full
explanation, and `scripts/test-fingerprint.mjs` for a regression test
covering this exact scenario.

Also added in this pass: library entries can now be **deleted** (there
was previously no way to remove one at all) — needed both to clean up any
stuck pre-fix entries and just generally useful.
...
filename, size, and duration — not the audio itself. (Duration is
included for informational display, e.g. showing the file's length in the
library list before it's reconnected, but — see below — it's deliberately
**not** used to decide whether two files match.)
...
same exact byte size — the fingerprint matches and the imported
timestamps show up automatically, the same way reopening one of your own
files works.

**A real bug this design fixes:** the fingerprint used to also include the
file's *duration*, rounded to a tenth of a second. That broke in
practice: duration is measured by actually decoding the audio, and
different browsers/platforms can report meaningfully different durations
for the *exact same file* — especially for MP3, where duration is often
estimated rather than read from a reliable header field, and Safari/iOS's
decoder can disagree with another platform's by more than that rounding
absorbed. The symptom was exactly what you'd expect: import a cue sheet,
it reports success, but loading the (genuinely correct) audio file on iOS
still showed "Needs re-select" — the fingerprints silently didn't match.
Filename + exact byte size alone is already essentially impossible to
collide between two genuinely different recordings, so duration was
dropped from the fingerprint entirely rather than trying to widen the
tolerance further.

**Limits worth knowing:**
- The recipient's audio file needs to be the *exact* same file (same
  filename, same byte size) for this to auto-match — a re-exported or
  renamed copy of the "same" recording will register as a different file
  and won't connect automatically. This is the tradeoff the reconnect
  feature already accepts (see `fingerprint.js`).
